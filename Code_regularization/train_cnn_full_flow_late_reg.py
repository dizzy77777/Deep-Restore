...
if __name__ == '__main__':
   ...
   # sort out black masks for training
    ...
    for layer in range(0, layers):
        ...
        dw_h_convs1[layer] = tf.nn.relu(conv1_bn + b1)
        print("layer:",layer, "conv1:", conv1.get_shape())
        conv1_test = conv2d(in_node1_test, w1, 1.0)
        conv1_test_bn = tf.layers.batch_normalization(inputs=conv1_test, axis=-1, momentum=0.9, epsilon=0.001, center=True, scale=True, training = is_training)
        dw_h_convs1_test[layer] = tf.nn.relu(conv1_test_bn + b1)
        ...
        dw_h_convs2[layer] = tf.nn.relu(conv2_bn + b1)
        
        conv2_test = conv2d(in_node1_test, w1, 1.0)
        conv2_test_bn = tf.layers.batch_normalization(inputs=conv2_test, axis=-1, momentum=0.9, epsilon=0.001, center=True, scale=True, training = is_training)
        dw_h_convs2_test[layer] = tf.nn.relu(conv2_test_bn + b1)
        ...
        dw_h_convs3[layer] = tf.nn.relu(conv3_bn + b1)
        
        conv3_test = conv2d(in_node1_test, w1, 1.0)
        conv3_test_bn = tf.layers.batch_normalization(inputs=conv3_test, axis=-1, momentum=0.9, epsilon=0.001, center=True, scale=True, training = is_training)
        dw_h_convs3_test[layer] = tf.nn.relu(conv3_test_bn + b1)
        ...
        if layer < layers-1:
            ...
            in_node1_test = dw_h_convs1_test[layer]
            in_node2_test = dw_h_convs2_test[layer]
            in_node3_test = dw_h_convs3_test[layer]
            
            
    print(dw_h_convs1[layers-1].get_shape())
    in_node = tf.concat([dw_h_convs1[layers-1], dw_h_convs2[layers-1], dw_h_convs3[layers-1]], 3)
    print("con cat:", in_node.get_shape())
    in_node_test = tf.concat([dw_h_convs1_test[layers-1], dw_h_convs2_test[layers-1], dw_h_convs3_test[layers-1]], 3)
    ...
    for layer in range(0, layers):
        ...
        print("layer:",layer, "conv:", conv.get_shape())
        conv_bn = tf.layers.batch_normalization(inputs=conv, axis=-1, momentum=0.9, epsilon=0.001, center=True, scale=True, training = is_training)
        dw_h_convs_comb[layer] = tf.nn.relu(conv_bn + b1)
        
        conv_test = conv2d(in_node, w1, 1.0)
        conv_test_bn = tf.layers.batch_normalization(inputs=conv_test, axis=-1, momentum=0.9, epsilon=0.001, center=True, scale=True, training = is_training)
        dw_h_convs_comb_test[layer] = tf.nn.relu(conv_test_bn + b1)
        ...
        if layer < layers-1:
            ...
            in_node_test = dw_h_convs_comb_test[layer]

    in_node = dw_h_convs_comb[layers-1]
    in_node_test = dw_h_convs_comb_test[layers-1]
    ...
    conv_test = conv2d(in_node_test, weight, 1.0)
    pred_mask_test = tf.nn.sigmoid(conv_test + bias)
    ...
    loss_sum = tf.summary.scalar("cross_entropy_late", loss)
    
    
    loss_batch = tf.placeholder(tf.float32)
    loss_batch_summary = tf.summary.scalar("cross_entropy_late", loss_batch)
    ...
    accuracy_sum = tf.summary.scalar("accuracy_late", accuracy)
    
    accuracy_batch = tf.placeholder(tf.float32)
    accuracy_batch_summary = tf.summary.scalar("accuracy_late", accuracy_batch)
    ...
    model_directory = os.path.join(current_directory, r'late')
