...
if __name__ == '__main__':
    ...
    loss_sum = tf.summary.scalar("cross_entropy_late_late", loss)
    
    
    loss_batch = tf.placeholder(tf.float32)
    loss_batch_summary = tf.summary.scalar("cross_entropy_late_late", loss_batch)
    ...
    accuracy_sum = tf.summary.scalar("accuracy_late_late", accuracy)
    
    accuracy_batch = tf.placeholder(tf.float32)
    accuracy_batch_summary = tf.summary.scalar("accuracy_late_late", accuracy_batch)
